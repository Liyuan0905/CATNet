...
class SalObjDatasetRGB(data.Dataset):
    def __init__(self, image_root, gt_root, ratio, trainsize):
    ...
    def __getitem__(self, index):
        ...
        image_ch = colorEnhance(image)
        # gt = randomPeper(gt)
        image = self.img_transform(image)
        image_ch = self.img_transform(image_ch)
        
        gt = self.gt_transform(gt)
        return image, image_ch, gt, index
...
class Confidence(data.Dataset):
    def __init__(self, image_root, gt_root, trainsize):
        # image_root : the Labeled Datasets ground-truth
        # gt_ root : the pseudo-labels
...
class SalObjDatasetTrip(data.Dataset):
    def __init__(self, image_root, gt_root, psd_root, trainsize):
        self.trainsize = trainsize
        self.images = [image_root + f for f in os.listdir(image_root) if f.endswith('.jpg')]
        self.gts = [gt_root + f for f in os.listdir(gt_root) if f.endswith('.jpg') or f.endswith('.png')]
    ...
    def __getitem__(self, index):
        ...
        image, gt, psd = cv_random_flip_trip(image, gt, psd)
        image, gt, psd = randomCrop_trip(image, gt, psd)
        image, gt, psd = randomRotation_trip(image, gt, psd)
        image = colorEnhance(image)
        gt = randomPeper(gt)
        psd = randomPeper(psd)
        image = self.img_transform(image)
        gt = self.gt_transform(gt)
        psd = self.psd_transform(psd)
        
        return image, gt, psd, index

    def filter_files(self):
        assert (len(self.images) == len(self.gts)) and (len(self.gts) == len(self.psds))
        images = []
        gts = []
        psds = []
        for img_path, gt_path, psd_path in zip(self.images, self.gts, self.psds):
            img = Image.open(img_path)
            gt = Image.open(gt_path)
            psd = Image.open(psd_path)
            
            if (img.size == gt.size) and (gt.size == psd.size):
                images.append(img_path)
                gts.append(gt_path)
    ...
    def resize(self, img, gt, psd):
        assert (img.size == gt.size) and (gt.size == psd.size) 
        w, h = img.size
        if h < self.trainsize or w < self.trainsize:
            h = max(h, self.trainsize)
            w = max(w, self.trainsize)
            return img.resize((w, h), Image.BILINEAR), gt.resize((w, h), Image.NEAREST), psd.resize((w, h), Image.NEAREST)
        else:
            return img, gt, psd
...
def get_loader(image_root, gt_root, ratio, batchsize, trainsize, shuffle=True, num_workers=12, pin_memory=True):
    dataset = SalObjDatasetRGB(image_root, gt_root, ratio, trainsize=trainsize)
    ...
    train_sampler, train_remain_sampler = data.random_split(dataset, [label_set_size, unlabel_set_size])
...
def get_loader_psd(image_root, psd_root, ratio, batchsize, trainsize, shuffle=True, num_workers=12, pin_memory=True):
    dataset = SalObjDatasetRGB(image_root, psd_root, ratio, trainsize=trainsize)
    partial_size = int(ratio * dataset.size)
    # pseudoGT_dataset = test_dataset(image_root, testsize=trainsize)
    ...
    train_sampler = data.sampler.SubsetRandomSampler(train_img_idx[:partial_size])
    train_remain_sampler = data.sampler.SubsetRandomSampler(train_img_idx[partial_size:])
    
    data_loader = data.DataLoader(dataset=dataset, batch_size=batchsize, sampler=train_sampler, num_workers=num_workers, pin_memory=pin_memory)
    data_remain_loader = data.DataLoader(dataset=dataset, batch_size=batchsize, sampler=train_remain_sampler, num_workers=num_workers, pin_memory=pin_memory)      
...
def get_loader_trip(image_root, gt_root, psd_root, ratio, batchsize, trainsize, shuffle=True, num_workers=12, pin_memory=True):
    dataset = SalObjDatasetTrip(image_root, gt_root, psd_root, trainsize=trainsize)
    partial_size = int(ratio * dataset.size)
    ...
    spv_data_loader = data.DataLoader(dataset=dataset, batch_size=batchsize, sampler=train_sampler, num_workers=num_workers, pin_memory=pin_memory)
    wspv_data_loader = data.DataLoader(dataset=dataset, batch_size=batchsize, sampler=train_remain_sampler, num_workers=num_workers, pin_memory=pin_memory)      
    
    return spv_data_loader, wspv_data_loader


def get_confidence(image_root, gt_root, ratio, batchsize, trainsize, shuffle=True, num_workers=12, pin_memory=True):
    dataset = Confidence(image_root, gt_root, trainsize=trainsize)
    partial_size = int(ratio * dataset.size)
    ...
    train_sampler = data.sampler.SubsetRandomSampler(train_img_idx[:partial_size])
    confidence_loader = data.DataLoader(dataset=dataset, batch_size=batchsize, sampler=train_sampler, num_workers=num_workers, pin_memory=pin_memory)
    
    return confidence_loader, len(confidence_loader)
